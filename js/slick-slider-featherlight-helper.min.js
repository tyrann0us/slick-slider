...
a){$(a).find(s.slickSliderLinkSelector).filter(slickSliderFeatherlightHelper.testImages).featherlightGallery({afterContent:function(){var a=this.$instance,b=this.$currentTarget,c=b.parent().find(s.slickSliderCaptionSelector);s.targetSlider=b.parents(s.slickSliderSelector);a.find(".caption").remove();c.length&&$('<div class="caption">').text(c.text()).appendTo(a.find(s.featherlightContentSelector))},afterClose:function(){s.targetSlider=null}})},testImages:function(b,a){return/(.png|.jpg|.jpeg|.gif|.tiff|.bmp)$/.test($(a).attr("href").toLowerCase().split("?")[0].split("#")[0])},
featherlightNavigationCallback:function(){var b=this,a=$.featherlight.prototype.namespace,a="previous."+a+" next."+a;$(document).on(a,s.featherlightNavigationArrowsSelector,function(a){b.slickSliderGoTo(a)});$(document).on("previous next",s.featherlightInstanceSelector,function(a){b.slickSliderGoTo(a)})},slickSliderGoTo:function(b){if(s.targetSlider){var a=s.targetSlider.slick("slickCurrentSlide");s.targetSlider.slick("slickGoTo","next"===b.type?a+1:a-1)}},init:function(){$=jQuery;s=this.settings;
this.findSliders();this.featherlightNavigationCallback()}};jQuery(document).ready(function(){slickSliderFeatherlightHelper.init()});